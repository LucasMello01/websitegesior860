  ...
  return $offer_array;
	...
	<center><font color="green"><i>No momento voce possue <u><b>'.$user_premium_points.'</b></u> Pontos</font></i></center></td></tr></td></tr>
		...
		if(count($offer_list['pacc']) > 0 )
				$main_content .= '<center><table border="0" cellpadding="1" cellspacing="1" width="650" align="center"><tr width="650" bgcolor="#505050"><td colspan="3"><font color="white" size="4"><b>&nbsp;VIP</b></font></td></tr><tr bgcolor="#D4C0A1"><td width="50" align="center"><b>Item</b></td><td width="350" align="left"><b>Descricao</b></td><td width="250" align="center"><b>Comprar</b></td></tr>';
				...
				if(count($offer_list['pacc']) > 0)
					foreach($offer_list['pacc'] as $pacc) {
						...
						$main_content .= '<form action="index.php?subtopic=shopsystem&action=select_player" method=POST><input type="hidden" name="buy_id" value="'.$pacc['id'].'"><input type="submit" value="Comprar"><br><b></b></form>';
					}
			...
			if(count($offer_list['item']) > 0 or count($offer_list['container']) > 0 or count($offer_list['itemlogout']) > 0)
				...
				if(count($offer_list['itemlogout']) > 0) {
					...
					foreach($offer_list['itemlogout'] as $itemlogout) {
						...
						if(!$logged) $main_content .= '<b>Login to buy</b>'; else 
						$main_content .= '<form action="index.php?subtopic=shopsystem&action=select_player" method=POST><input type="hidden" name="buy_id" value="'.$itemlogout['id'].'"><input type="submit" value="Buy '.$itemlogout['name'].'"><br><b></b></form>';
						$main_content .= '</td></tr>';
					}
					$main_content .= '</table><br />';
				}
			if(count($offer_list['changename']) > 0 or count($offer_list['redskull']) > 0 or count($offer_list['unban']) > 0 )
				...
				if(count($offer_list['changename']) > 0)
					foreach($offer_list['changename'] as $changename) {
						...
						if(!$logged) $main_content .= '<b>Login to buy</b>'; else 
						$main_content .= '<form action="index.php?subtopic=shopsystem&action=select_player" method=POST><input type="hidden" name="buy_id" value="'.$changename['id'].'"><input type="submit" value="Buy '.$changename['name'].'"><br><b></b></form>';
					}
				//Remove Red Skull
				if(count($offer_list['redskull']) > 0)
					foreach($offer_list['redskull'] as $redskull) {
						...
						if(!$logged) $main_content .= '<b>Login to buy</b>'; else 
						$main_content .= '<form action="index.php?subtopic=shopsystem&action=select_player" method=POST><input type="hidden" name="buy_id" value="'.$redskull['id'].'"><input type="submit" value="Buy '.$redskull['name'].'"><br><b></b></form>';
						$main_content .= '</td></tr>';
					}
				//Unban
