...
if($action == "")
{
	...
	$main_content .= 'Para jogar no '.$config['server']['serverName'].' Voce precisa de um acesso. 
		Tudo que voce precisa para se criar uma nova conta, Obrigatorio seu endereco de e-mail (VALIDO), senha para novo codigo de verificacao de conta, a partir de imagem e de concordar com os termos apresentados abaixo.
Se voce tiver feito isso, seu numero de conta, senha e e-mail sera mostrado em sua caixa de e-mail, nao use e-mail falso pois nao consiguira criar uma conta em nosso server.
para o seu enderebo de e-mail junto com instrucoes adicionais.<BR><BR>
		<FORM ACTION="index.php?subtopic=createaccount&action=saveaccount" onsubmit="return validate_form(this)" METHOD=post>
			<TABLE WIDTH=100% BORDER=0 CELLSPACING=1 CELLPADDING=4>
				...
				$main_content .= '
				<TR BGCOLOR="'.$config['site']['darkborder'].'">
					<TD colspan="2">Por favor revise os seguintes termos e indicar o seu contrato abaixo.</TD>
				</TR>
				<TR BGCOLOR="'.$config['site']['darkborder'].'">
					<TD colspan="2">
				...
				<TR BGCOLOR="'.$config['site']['darkborder'].'">
					<TD colspan="2">
						Se voce concorda plenamente com estes termos, clique em "Concordo" botao, a fim de criar uma '.$config['server']['serverName'].' account.<BR>
						Se voce nao concordar com esses termos ou nao quiser criar uma '.$config['server']['serverName'].' account, please click on the "Cancel" button.
					</TD>
				</TR>
			</TABLE><BR>
...
if($action == "saveaccount") 
{
	...
	## check name
	if(empty($reg_name))
		$reg_form_errors[] = "Por favor, insira o nome conta.";
	elseif(!check_account_name($reg_name))
		$reg_form_errors[] = "Formato de nome invalido conta. Utilize apenas AZ e números 0-9.";
	## check email
	if(empty($reg_email))
		$reg_form_errors[] = "Digite seu endereco de email.";
	else
	{
		if(!check_mail($reg_email))
			$reg_form_errors[] = "E-mail nao esta correto.";
	}
	...
	elseif(!$config['site']['create_account_verify_mail'])
	{
		if(!check_password($reg_password))
			$reg_form_errors[] = "Senha contem caracteres ilegais (az, AZ e 0-9 so!) Ou comprimento.";
	}
	//SECOND check
	...
	if(empty($reg_form_errors))
	{
		if($config['site']['one_email'])
		{
			...
			if($test_email_account->isLoaded())
				$reg_form_errors[] = "Conta com este endereco de e-mail ja existem no banco de dados.";
		}
		$account_db = new OTS_Account();
		$account_db->find($reg_name);
		if($account_db->isLoaded())
			$reg_form_errors[] = 'Conta com este nome ja existe.';
		if($config['site']['referrer'])
		{
			$ref_check = $SQL->query('SELECT id FROM accounts WHERE id = '.$reg_referrer.' LIMIT 1')->fetch();
			if (($reg_referrer > 0) && ($ref_check['id'] < 1))
			{
				$reg_form_errors[] = 'O uso do numero do jogador que voce se refere ao nosso servidor nso existe. Pergunte a ele por um link valido ou registro de re aos registos sem erro referência.';
			}
		}
	}
	...
	if(empty($reg_form_errors))
	{
		...
		$main_content .= 'Sua conta foi criada. Por favor, anote o numero da conta e senha. Vejo voces no Even War!<BR><BR>
		<TABLE WIDTH=100% BORDER=0 CELLSPACING=1 CELLPADDING=4>
			<TR>
			...
			<TR>
				<TD BGCOLOR="'.$config['site']['darkborder'].'">
					<TABLE BORDER=0 CELLPADDING=1>
						<TR>
							<TD>
								<FONT SIZE=5>Write down your account name: <B>'.$reg_name.'</B></FONT><BR><BR>Seu password e <b>'.trim($_POST['reg_password']).'</b>.
								Voce tera o nome da conta e sua senha para jogar em '.$config['server']['serverName'].'.
								Por favor, mantenha o nome da conta ea senha em um lugar seguro e nunca dar o seu nome de conta ou senha para ninguem.<BR><BR>';
								if($config['site']['send_emails'] && $config['site']['create_account_verify_mail'])
								{
									$mailBody = '<html>
									<body>
									<h3>O nome da conta e a senha!</h3>
									<p>Voce ou alguem ja esta registado no servidor <a href="http://www.arenatibia.net"><b>'.$config['server']['serverName'].'</b></a> com este e-mail.</p>
									<p>Account name: <b>'.$reg_name.'</b></p>
									<p>Password: <b>'.trim($reg_password).'</b></p>
									<br />
									<p>Apos o login voce pode:</p>
									<li>Criar novos characters
									<li>Alterar sua senha atual
									<li>Mude o seu e-mail atual
									</body>
									</html>';
									...
									if($mail->Send())
									{
										$main_content .= 'Sua conta foi criada. Verifique seu e-mail. Vejo voces no Even War!';
									}
									else
									{
										$main_content .= '<br /><small>Um erro durante o envio de e-mail occorred! Conta nao criada. tente novamente.</small>';
										$reg_account->delete();
									}
								}
								elseif($config['site']['send_emails'] && $config['site']['send_register_email'])
								{
									$mailBody = '<html>
									<body>
									<h3>O nome da conta e senha!</h3>
									<p>Voce ou alguem ja esta registado no servidor <a href="'.$config['server']['url'].'"><b>'.$config['server']['serverName'].'</b></a> with this e-mail.</p>
									<p>Account name: <b>'.$reg_name.'</b></p>
									...
									$mail->Body = $mailBody;
									if($mail->Send())
										$main_content .= '<br /><small>Estas informacoes foram em enviar e-mail <b>'.$reg_email.'</b>. Please check your inbox.';
									else
										$main_content .= '<br /><small>Um erro durante o envio de e-mail occorreu!';
								}
									$main_content .= '
							</TD>
						</TR>
